...
function display(rackSide, data) {
    ...
    for(unit = 42; unit > 0; unit--) {
        ...
        $("#unitcount1").css("font-size", "16.6px");
    }

    Object.keys(data['dcim_data']).forEach(function(row_num) {
        //Row(numRacks, clusterName, information)
        let row = new Row(8, "Lemhi", "something here");
        $("#rowarea").append(`<div id="rowarea${row_num}" class="col-sm-10" style="position:absolute"></div>`);
        ...
        Object.keys(data['dcim_data'][row_num]).forEach(function(rack_num) {
            ...
            let rack_base = new Rack(rack[0]['dcim_row'], rack[0]['dcim_rack'], 42, rackStartLeft(rack[0]['dcim_rack']), rackStartLeft(rack[0]['dcim_rack']), rackHeight);
                ...
                if(rackSide.includes('front')) {
                    ...
                    if(chassis['hardware_type'] === 'compute') {
                        ...
                        chassis['nodes'].forEach(function(node) {
                            ...
                            subnodeArray.push(new Subnode(nodeCount, Number(rackUnitHeight(2).replace("px", "")) - 27, subnodeWidth, node['dcim_left_start'], node['dcim_top_start'], node['information'], node['display_name']));
                            nodeCount += 1;
                        });
                        rack_base.addChassis(new Chassis(chassis['dcim_unit_start_location'],
                            chassis['chassis_height'], rackUnitHeight(chassis['chassis_height']),
