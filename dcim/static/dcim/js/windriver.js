...
function display(rackSide, data) {
    ...
    Object.keys(data['dcim_data']).forEach(function(row_num) {
        ...
        let row = new Row(8, "WindRiver", "something here");
        let rackcounter = 1;
        Object.keys(data['dcim_data'][row_num]).forEach(function(rack_num) {
            ...
            if(row_num === '1') {
                rack_base = new Rack(row_num, rack[0]['dcim_rack'], 48, rackStartLeft(dcim_rack), rackStartLeft(dcim_rack), rackHeight);
            } else {
                rack_base = new Rack(row_num, rack[0]['dcim_rack'], 48, rackStartLeft(rackcounter), rackStartLeft(rackcounter), rackHeight);
                dcim_rack = rackcounter;
                rackcounter++;
            }
            ...
            rack.forEach(function(chassis){
                ...
                if(rackSide.includes('front')) {
                    ...
                    if(chassis['hardware_type'] === 'compute') {
                        let subnodeArray = [];
                        let subnodeWidth = 82;
                        let subnodeHeight = Number(rackUnitHeight(2).replace("px", "")) - 27;
                        nodeCount = 0;
                        chassis['nodes'].forEach(function(node) {
                            //Subnode(subnodeNum, subnodeHeight, subnodeWidth, subnodeLeft, subnodeTop, information)
                            subnodeArray.push(new Subnode(nodeCount, subnodeHeight, subnodeWidth, node['dcim_left_start'], node['dcim_top_start'], chassis['display_name'] + '<br>' + node['information'], node['display_name']));
                            nodeCount += 1;
                        });
                        rack_base.addChassis(new Chassis(chassis['dcim_unit_start_location'],
                            chassis['chassis_height'], rackUnitHeight(chassis['chassis_height']),
